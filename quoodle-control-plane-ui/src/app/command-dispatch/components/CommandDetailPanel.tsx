...
export default function CommandDetailPanel({ command, onClose, onRetry }: CommandDetailPanelProps) {
  ...
  const timelineSteps = [
    {
      ...
      label: 'Queued',
      time: command.queuedAt,
      done: Boolean(command.queuedAtIso),
      desc: `Queued by ${command.actor}`,
    },
    {
      state: 'dispatched' as const,
      label: 'Dispatched',
      time: command.dispatchedAt,
      done: Boolean(command.dispatchedAtIso) || currentIndex >= 1,
      desc: 'Sent to FastAPI gateway',
    },
    {
      state: 'ack_received' as const,
      label: 'ACK Received',
      time: command.ackAt,
      done: Boolean(command.ackAtIso) || currentIndex >= 2,
      desc: 'Agent acknowledged receipt',
    },
    {
      ...
      desc: `Running method ${command.method}`,
    },
    {
      state: 'completed' as const,
      label: 'Completed',
      time: command.completedAt,
      done: Boolean(command.completedAtIso) || currentIndex >= 4,
      desc: 'Terminal state',
    },
  ];
      ...
      <div className="fixed inset-y-0 right-0 w-full max-w-md bg-zinc-950 border-l border-border z-50 flex flex-col slide-in-right">
          ...
          <div>
            ...
            <div className="space-y-0">
              {timelineSteps.map((step, index) => {
                const isReached = !isFailed && step.done;
                const isActive = !isFailed && currentIndex === index && command.state !== 'completed';
              ...
              {isFailed && (
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(command.errorMessage || command.reason) && (
                      <p className="text-[11px] text-red-400/80 mt-1">{command.errorMessage ?? command.reason}</p>
                    )}
                  </div>
                </div>
              )}
          ...
          <div className="bg-muted/20 border border-border rounded-lg p-3">
            ...
            <div className="space-y-1">
              {[
                ...
                { k: 'device_id', v: command.deviceId },
                { k: 'method', v: command.method },
                { k: 'queued_at', v: command.queuedAt || '-' },
                { k: 'dispatched_at', v: command.dispatchedAt ?? '-' },
                { k: 'ack_at', v: command.ackAt ?? '-' },
                { k: 'completed_at', v: command.completedAt ?? '-' },
              ].map((row) => (
        ...
        <div className="border-t border-border px-5 py-3 flex items-center gap-2">
          ...
          {command.state === 'queued' && (
            <button
              onClick={() => toast.warning(`Cancel requested for ${command.id}`)}
          ...
          <button
            onClick={async () => {
              await navigator.clipboard.writeText(command.traceId);
              toast.info('Copied trace ID');
            }}
